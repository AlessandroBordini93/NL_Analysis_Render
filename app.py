...
#
# NEW (richiesta):
# - possible_steps nel JSON (settings.possible_steps):
#   * default: [25,50,75,100] se non presente
#   * sono ammessi solo {25,50,75,100} (qualsiasi combinazione)
#   * PASSO effettivo = min(possible_steps) (coerenza “multipli del passo base”)
#   * se possible_steps = [25,50,75,100] e PASSO=25 => logica intermedie IDENTICA a oggi
#   * altrimenti: le linee secondarie vengono generate decomponendo ogni intervallo rem
#     usando SOLO i passi in possible_steps e minimizzando l’uso del 25 (se presente)
#   * se in un intervallo non esiste decomposizione => ERRORE
#
# NEW (FULL - richiesto):
# - FULL (solo quando buildingType in input è "cemento_armato"):
#   * suddivisioni primarie/finestre/sec IDENTICHE al caso con aperture (le finestre “guidano” Xall/Yall)
#   * poi rinforzo FULL continuo: niente clipping contro finestre (come se sparissero)
#   * calcolo Aeq/Keq per pannello + Aeq univoca per FULL
#   * output JSON: results.full (dati e liste coordinate) + overlay "grid_full"
#
# NEW (aggiunta richiesta UTENTE):
# - results.passo_medio_mm: dizionario { "i,j": valore_mm } per tutti i pannelli,
#   dove valore_mm = ((passo_medio_x_cm + passo_medio_y_cm)/2) * 10
#   calcolato per ciascun pannello usando SOLO Xall/Yall (quindi uguale OPEN/FULL).
...
app = FastAPI(
    title="RebarCA API",
    version="2.12 (FULL cemento_armato + possible_steps + schema_scale query param + muratura page clean + passo_medio_mm per pannello)",
)
...
# ============================================================
# INTERMEDIE (SECONDARIE) — possible_steps
# ============================================================
def _normalize_possible_steps(possible_steps: Optional[List[int]]) -> List[int]:
    ...
    if not steps:
        raise ValueError("settings.possible_steps vuoto: impossibile generare linee secondarie")
...
def intermedie(lines: List[float], PASSO: int, possible_steps: Optional[List[int]] = None) -> List[float]:
    ...
    for a, b in zip(lines[:-1], lines[1:]):
        ...
        if dp_cost[target_units] == INF:
            raise ValueError(
                f"Impossibile generare linee secondarie: intervallo {rem:.3f} cm non decomponibile con possible_steps={steps}"
            )
...
def parse_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    PASSO = int(min(possible_steps))  # richiesto
    ...
    # Normalizzazione BEAMS: TOP->BOTTOM => BOTTOM->TOP
...
def compute(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # FIX: elimina NON primarie dentro o SUI BORDI di pilastri/travi
    EPSG = 1e-6
    ...
    # intermedie
    Xsec = intermedie(sorted(set(Xbase + Xfin)), PASSO=PASSO, possible_steps=possible_steps)
    Ysec = intermedie(sorted(set(Ybase + Yfin)), PASSO=PASSO, possible_steps=possible_steps)
    ...
    # V/H raw con clipping vs finestre gonfiate clampate (DISTF=0 perché già gonfiate)
    v_raw: List[Tuple[float, float, float]] = []
    h_raw: List[Tuple[float, float, float]] = []
    ...
    # stiffness OPEN (finestre già gonfiate+clampate => DISTF=0)
    ...
    # ============================================================
    # NEW: PASSO MEDIO PER PANNELLO (in mm) — usando Xall/Yall
    # ============================================================
    ...
    for i in range(nt):
        for j in range(np__):
            ...
            Xp = [x for x in Xall if (xL - eps) <= x <= (xR + eps)]
            Yp = [y for y in Yall if (yB - eps) <= y <= (yT + eps)]

            dxp = [Xp[k + 1] - Xp[k] for k in range(len(Xp) - 1)]
            dyp = [Yp[k + 1] - Yp[k] for k in range(len(Yp) - 1)]

            mx = (sum(dxp) / len(dxp)) if dxp else 0.0
            my = (sum(dyp) / len(dyp)) if dyp else 0.0

            pm_cm = (mx + my) / 2.0 if (dxp and dyp) else (mx or my or 0.0)
            pm_mm = pm_cm * 10.0

            passo_medio_dict[(i, j)] = float(round(pm_mm, 3))
    ...
    # ============================================================
    # FULL (solo cemento_armato): stesso Xall/Yall (guidati da finestre)
    # ma rinforzo continuo (nessun clipping)
    # ============================================================
    full_block: Optional[Dict[str, Any]] = None
    if cfg.get("building_type") == "cemento_armato":
        # costruisco V/H continui e diagonali complete su ogni cella
    ...
    out = {
        ...
        "results": {
            ...
            # NEW: stessa struttura di Aeq_by_panel_mm2
